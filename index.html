...
<head>
  ...
  <title>Auto-Healing Dashboard — ToDo • Weather • Query</title>
  <style>
    :root{
...
<body>
  <div class="app">
    <header>
      <div>
        <h1>Auto‑Healing Dashboard</h1>
        <div class="meta">To-Do • Weather • Smart Query — simulated auto-heal</div>
      </div>
      <div class="meta small">User: NIKHILUTTAM • Demo</div>
    </header>
    ...
    <div class="grid">
      ...
      <section class="card">
        ...
        <div style="margin-top:12px" class="small">Cached data age: <span id="cacheAge">—</span></div>
      </section>
  ...
  <script>
    ...
    async function loadTasks(){
      try{
        ...
        $('healMessage').textContent = '⚠️ API failed — Auto‑healed using cached tasks.';
    ...
    function renderTasks(tasks){
      ...
      tasks.forEach((t,i)=>{
        ...
        li.innerHTML = `<div>${escapeHtml(t)}</div><div class='task-actions'><button onclick="deleteTask(${i})">✖</button></div>`;
        ul.appendChild(li);
      })
    }
    ...
    async function refreshWeather(){
      try{
        ...
        if(cached){ showWeather(cached.data,'healed'); recordHeal('weather',err);} else { showWeather({temp:'—',desc:'Unavailable'},'error'); recordHeal('weather',err);}      
      }
    }
    ...
    function showWeather(data,state){
      ...
      $('weatherSummary').textContent = data.desc || '—';
      $('tempLarge').textContent = (data.temp===undefined? '—' : data.temp + '°C');
      const c = getWeatherCache(); $('cacheAge').textContent = c? Math.round((Date.now()-c.ts)/1000) + 's' : '—';
    }
    ...
    document.addEventListener('DOMContentLoaded', ()=>{
      ...
      $('clearWeatherCache').addEventListener('click', ()=>{ localStorage.removeItem(WKEY); showWeather({temp:'—',desc:'—'},'error'); });
